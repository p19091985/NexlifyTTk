...
try:
    ...
    messagebox.showerror("Erro Crítico de Importação", f"Não foi possível importar módulos essenciais: {e}.\n\nVerifique se o script está na pasta correta.")
    sys.exit(1)
...
class ConnectionManagerFrame(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        self.file_status_label = ttk.Label(controls_frame, text="Nenhum arquivo carregado.")
        ...
        self.ini_preview_area = scrolledtext.ScrolledText(preview_frame, wrap=tk.WORD, state="disabled", font=FONT_COURIER)
    ...
    def _parse_and_populate_ui(self):
        ...
        for line in self.ini_lines:
            ...
            if match:
                ...
                current_db = db_name
                if current_db not in self.db_configs: self.db_configs[current_db] = {'active': False}
            clean_line = line.strip()
            if current_db and not clean_line.startswith(('#', ';')) and '=' in clean_line:
                key = clean_line.split('=', 1)[0].strip().lower()
                if key == 'type': active_db = current_db
        if active_db: self.db_configs[active_db]['active'] = True
        for widget in self.db_options_frame.winfo_children(): widget.destroy()
        ...
        for db_type, config_info in sorted(self.db_configs.items()):
            ...
            if config_info['active']:
                ...
                btn = ttk.Button(card, text="Ativar", command=lambda dt=db_type: self._activate_db(dt))
                btn.pack(side="right")
    ...
    def _activate_db(self, db_to_activate: str):
        ...
        if not messagebox.askyesno("Confirmar Ativação", msg, parent=self): return
        new_lines = []
        current_db = None
        marker_pattern = re.compile(r"\s*Configuração para\s+(.*?)\s*", re.IGNORECASE)
        for line in self.ini_lines:
            ...
            if current_db and key_match and key_match.group(1).lower() in DB_CONFIG_KEYS:
                ...
                new_lines.append(clean_line if current_db == db_to_activate else f"#{clean_line}")
            else:
        ...
        try:
            ...
            self._parse_and_populate_ui()
            self._update_ini_preview()
            messagebox.showinfo("Sucesso", f"Conexão '{db_to_activate.capitalize()}' ativada!\nBackup criado em: {backup_path.name}", parent=self)
        except Exception as e:
...
class CryptoManagerFrame(ttk.Frame):
    def __init__(self, parent, *args, **kwargs):
        ...
        ttk.Button(hash_gen_frame, text="Gerar Hash", command=self._generate_and_display_hash).grid(row=0, column=2, padx=5)
        ...
        ttk.Button(hash_gen_frame, text="Copiar", command=self._copy_generated_hash).grid(row=1, column=2, padx=5, pady=(5, 0))
        ...
        ttk.Button(log_frame, text="Copiar Log", command=self._copy_log).grid(row=1, column=0, sticky="ew", pady=(10, 0))
    ...
    def _encrypt_ini_file(self):
        ...
        try:
            ...
            for line in lines:
                ...
                if key_match:
                    prefix, key_name, separator, value = key_match.groups()
                    if value and not value.startswith("gAAAAA"):
                        encrypted_value = encrypt_message(value, key)
                        new_lines.append(f"{prefix}{key_name}{separator}{encrypted_value}")
                        self._log(f"Campo '{key_name}' criptografado.", "success")
                        changed = True
                    else:
                        new_lines.append(line)
                else:
            ...
            if changed:
                ...
                messagebox.showinfo("Nenhuma Alteração", "O arquivo já parece estar totalmente criptografado. Nenhuma alteração foi necessária.", parent=self)
        except Exception as e:
    ...
    def _update_sql_hashes(self):
        ...
        try:
            ...
            pattern = re.compile(r"(INSERT\s+INTO\s+USUARIOS.*?VALUES\s*\(.*?,)\s*'(.*?)'(\s*,.*?\))", re.IGNORECASE | re.DOTALL)
            def replacer(match):
            ...
            if count > 0:
                ...
                messagebox.showinfo("Nenhuma Alteração", "Nenhuma senha em texto plano foi encontrada nos comandos INSERT. O arquivo não foi modificado.", parent=self)
        except Exception as e:
            self._log(f"ERRO: {e}", "error")

class CredentialViewerFrame(ttk.Frame):
    SENHAS_CONHECIDAS = {'admin': 'admin', 'diretor.op': 'dir123', 'gerente.ti': 'ti123', 'ana.supervisor': 'sup123', 'bruno.operador': 'op123', 'carla.analista': 'ana123', 'davi.auditor': 'auditor123', 'elisa.op': 'op456', 'felipe.gerente': 'ti456', 'gabi.analista': 'ana456', 'hugo.diretor': 'dir456', 'isa.supervisor': 'sup456', 'joao.operador': 'op789', 'lara.admin': 'admin456', 'mateus.auditor': 'auditor456'}
    def __init__(self, parent, *args, **kwargs):
        super().__init__(parent, *args, **kwargs)
        self.columnconfigure(0, weight=1)
        self.rowconfigure(1, weight=1)
        ttk.Button(self, text="Analisar Arquivo de Credenciais (.ini ou .sql)", command=self._analyze_file).grid(row=0, column=0, sticky="ew", pady=(0, 10))
    ...
    def _analyze_file(self):
        filepath = filedialog.askopenfilename(title="Analisar Arquivo", filetypes=[("Arquivos Suportados", "*.ini *.sql"), ("Arquivos INI", "*.ini"), ("Arquivos SQL", "*.sql"), ("Todos os arquivos", "*.*")])
        if not filepath: return
        for item in self.tree.get_children(): self.tree.delete(item)
        file = Path(filepath)
        if file.suffix == '.ini': self._analyze_ini(file)
        elif file.suffix == '.sql': self._analyze_sql(file)
    ...
    def _analyze_sql(self, file: Path):
        try:
            ...
            found_users = False
            insert_block_match = re.search(r"INSERT\s+INTO\s+USUARIOS.*?VALUES.*?;", content, re.IGNORECASE | re.DOTALL)
            if insert_block_match:
                insert_block_text = insert_block_match.group(0)
                user_tuple_pattern = re.compile(r"\(\s*'([^']*)'\s*,\s*'([^']*)'\s*,\s*'[^']*'\s*,\s*'[^']*'\s*\)", re.IGNORECASE)
                for match in user_tuple_pattern.finditer(insert_block_text):
                    ...
                    if login in self.SENHAS_CONHECIDAS:
                        ...
                        try:
                            decrypted_value = known_password if check_password_hash(known_password, hash_senha) else "SENHA NÃO CONFERE!"
                        except Exception:
                            decrypted_value = "HASH INVÁLIDO"
                    self.tree.insert("", "end", values=(file.name, "INSERT USUARIOS", login, hash_senha, decrypted_value))
                    found_users = True
            create_pattern = re.compile(r"(CREATE\s+(USER|LOGIN))\s+.*?['\"](.*?)['\"].*?(IDENTIFIED\s+BY|WITH\s+PASSWORD)\s*=?\s*['\"](.*?)['\"]", re.IGNORECASE)
            for match in create_pattern.finditer(content):
                command, _, user, _, password = match.groups()
                self.tree.insert("", "end", values=(file.name, command.strip(), user, password, password))
                found_users = True
            if not found_users:
                messagebox.showinfo("Análise Concluída", "Nenhuma credencial de usuário reconhecida foi encontrada no arquivo.", parent=self)
        except Exception as e:
