# utilParaDesenvolvimento/toolsDev/generator_ui.py
...
class GeneratorApp(bstrap.Window):
    """A interface gráfica para o assistente de geração de código."""

    def __init__(self):
        super().__init__(themename="superhero", title="✨ Assistente de Geração Divina ✨")
        self.geometry("950x800")
        self.minsize(850, 700)
        self.generator = CodeGenerator(self.log)
        self.user_roles = ['Administrador Global', 'Diretor de Operações', 'Gerente de TI', 'Supervisor de Produção',
                           'Operador de Linha', 'Analista de Dados', 'Auditor Externo']
...
class ComponentBuilder(ttk.Frame):
    """Constrói a UI para uma aba do gerador (Painel ou Modal)."""
    ...
    def _create_basic_info_widgets(self):
        ...
        ttk.Combobox(frame, textvariable=self.panel_type_var, values=["Básico", "MVC CRUD"], state="readonly").grid(
    ...
    def _create_crud_widgets(self):
        ...
        ttk.Button(self.crud_frame, text="Carregar Tabelas do Banco de Dados", command=self.load_tables_from_db).grid(
        ...
        self.columns_text = scrolledtext.ScrolledText(self.crud_frame, height=5, width=40, state="disabled",
                                                      font=("Consolas", 9))
    ...
    def _create_action_button(self):
        self.generate_btn = ttk.Button(self, text=f"🚀 Gerar {self.component_type}", style="success.TButton",
    ...
    def run_generation(self):
        ...
        context = {
            ...
            'class_prefix': 'Painel' if self.component_type == "Painel" else '',
            'panel_type': self.panel_type_var.get(),
            'panel_name': panel_name,
            'panel_icon': self.panel_icon_var.get().strip(),
            'allowed_access': [role for role, var in self.access_vars.items() if var.get()]
        }
