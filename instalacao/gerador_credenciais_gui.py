...
try:
    ...
    messagebox.showerror("Erro Crítico de Importação",
                         ...
                         f"Verifique se este script está na pasta 'instalacao' e se a pasta 'persistencia' existe na raiz ({PROJECT_ROOT}).")
    sys.exit(1)
...
class CredentialToolApp(tk.Tk):
    ...
    def _setup_styles(self):
        """Configura estilos visuais modernos para a aplicação."""
    ...
    def _create_ini_tab(self, parent):
        """Cria os componentes da aba de geração de credenciais .ini."""
        ...
        ttk.Button(input_frame, text="Gerar Credenciais", command=self._generate_ini_creds,
                   style="Success.TButton").grid(row=3, column=0,
        ...
        self.ini_output_area = scrolledtext.ScrolledText(output_frame, height=5, font=FONT_CODE, wrap=tk.WORD,
    ...
    def _generate_ini_creds(self):
        """Lógica para criptografar o usuário e senha do .ini."""
    ...
    def _create_sql_tab(self, parent):
        ...
        ttk.Button(input_frame, text="Gerar Hash", command=self._generate_sql_hash, style="Success.TButton").grid(row=2,
                                                                                                                 column=0,
                                                                                                                 columnspan=2,
                                                                                                                 pady=10,
                                                                                                                 sticky="ew",
        ...
        ttk.Button(output_frame, text="Copiar Hash", command=self._copy_hash_output).grid(row=1, column=0, pady=(10, 0),
                                                                                          sticky="ew", ipady=5)

    def _generate_sql_hash(self):
        """Lógica para gerar o hash Bcrypt da senha."""
    ...
    def _copy_hash_output(self):
        """Copia o resultado .sql (hash) para a área de transferência."""
        self._copy_to_clipboard(self.sql_output_entry.get())

    def _toggle_sql_pass(self):
        """Alterna a visibilidade da senha na Aba 2."""
    ...
    def _create_tutorial_tab(self, parent):
        ...
        text_area.tag_add("h1", "1.0", "1.end")
        text_area.tag_add("code", "74.0", "78.end")
    ...
    def _copy_to_clipboard(self, content):
        ...
        if content:
            try:
                ...
                messagebox.showinfo("Sucesso", "Conteúdo copiado.", parent=self)
            except tk.TclError:
                messagebox.showwarning("Aviso", "Não foi possível acessar a área de transferência.", parent=self)
        else:
