...
class PainelGestaoLinguagens(BasePanel):
    ...
    def _create_form_widgets(self, parent):
        ...
        ttk.Spinbox(parent, from_=1950, to=2025, textvariable=self.ano_var, width=8).grid(row=0, column=4, sticky="w", padx=5, pady=5)
        ...
        ttk.Button(tipo_frame, text="➕", command=self._open_tipos_modal, width=3).pack(side="left", padx=(0, 5), pady=5)
        ...
        ttk.Button(btn_frame, text="Excluir", command=self._delete_item, style="Danger.TButton").pack(side="left", padx=5)
        ttk.Button(btn_frame, text="Limpar", command=self._clear_form, style="Secondary.TButton").pack(side="left", padx=5)
    ...
    def _create_table_widgets(self, parent):
        ...
        for col, width in col_widths.items(): self.tree.column(col, width=width, anchor='center' if col != 'nome' else 'w')
    ...
    def _create_legacy_action_buttons(self, parent):
        ttk.Button(parent, text="Reclassificar C++ para 'Legado' (Exemplo Atômico)", command=self._executar_reclassificacao, style="Info.TButton").pack(side="left", padx=5, pady=5)
    ...
    def _save_item(self):
        ...
        try:
            ...
            data = {'nome': nome, 'id_tipo': id_tipo, 'ano_criacao': self.ano_var.get(), 'categoria': self.categoria_var.get().strip()}
    ...
    def _delete_item(self):
        if self.selected_item_id is None:
            ...
            return
        if messagebox.askyesno("Confirmar Exclusão", "Deseja realmente excluir esta linguagem?", icon='warning', parent=self):
            try:
